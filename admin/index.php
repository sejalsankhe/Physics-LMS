<?php
$error=null;
if(isset($_POST['login']))
{
  include("../partials/dbconnect.php");
            $email=$db->real_escape_string($_POST['em']);
    $pass=$db->real_escape_string($_POST['pass']);
    
    
    $logsessionid="";
    
    
    $resultset=$db->query("SELECT * FROM admin WHERE email = '$email' AND password = '$pass'LIMIT 1");
    if($resultset->num_rows!=0) 
    {
        
        
       $row=$resultset->fetch_assoc();
        
        $username=$row['name'];
        $e=$row['email'];
        
        $id=$row['id'];
         session_start();
     $_SESSION['valid'] = "x";

        
      
     $_SESSION['name']=$username;

        $x=1;

        
        
         $em=$row['email'];
        
        
            
            header('location: area/index.php');
            

                  
        
             
             
           
             
        
    }
    else{
        $error="Invalid username or password"; 
    }
    
    
}


?>





<!DOCTYPE html>
<html lang="en">
<head>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
 
    <title>Admin</title>
   
   
   <!--Bootstrap Code-->
   <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
   <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
   <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
   <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
    <meta charset="UTF-8">
    <link href="../grade10/style.css" type="text/css" rel="stylesheet">
    
</head>
<body >
  <section class="studentlogin">

  <div class="jumbotron slj">
  <form method="post" action="">
  
  <div class="form-group">
    <label for="exampleInputEmail1">Email address</label>
    <input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter email" name="em">
    <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small>
  </div>
  <div class="form-group">
    <label for="exampleInputPassword1">Password</label>
    <input type="password" class="form-control" id="exampleInputPassword1" placeholder="Password" name="pass">
  </div>
  
  <button type="submit" class="b" name="login">login</button>
 
</form>
 
 <center>
     <?php
     echo $error;
     ?>
 </center>
  
</div>
   </section>

    
</body>
</html>